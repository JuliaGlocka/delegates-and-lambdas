...
{
    [TestCaseSource(typeof(DataSourceForTests), nameof(DataSourceForTests.DataForGenerateProgressionTestForIntegers))]
    [Category(nameof(GenerateProgression))]
    public void GenerateProgression_Generate_Integer_Elements_Progression(
        int first,
        Func<int, int> formula,
        int count,
        List<int> expected)
    {
        Assert.That(expected, Is.EqualTo(GenerateProgression(first, formula, count)));
    }

    [TestCaseSource(typeof(DataSourceForTests), nameof(DataSourceForTests.DataForGenerateProgressionTestForDoubles))]
    [Category(nameof(GenerateProgression))]
    public void GenerateProgression_Generate_Double_Elements_Progression(
        double first,
    ...
    [TestCaseSource(
        typeof(DataSourceForTests),
        nameof(DataSourceForTests.DataForGenerateProgressionTestWithConditionForIntegers))]
    [Category(nameof(GenerateProgression))]
    public void GenerateProgression_With_Predicate_Generate_Integer_Elements_Progression(
        int first,
        Func<int, int> formula,
        Predicate<int> finished,
        List<int> expected)
    {
        Assert.That(expected, Is.EqualTo(GenerateProgression(first, formula, finished)));
    }
    ...
    [Category(nameof(GenerateProgression))]
    public void GenerateProgression_With_Predicate_Generate_Double_Elements_Progression(
        double first,
        Func<double, double> formula,
        Predicate<double> finished,
        List<double> expected)
    {
        Assert.That(
            expected,
    ...
    {
        Assert.That(expected, Is.EqualTo(GetElement(first, formula, number)));
    }

    [TestCaseSource(typeof(DataSourceForTests), nameof(DataSourceForTests.DataForGetElementTestForDoubles))]
    [Category(nameof(GetElement))]
    public void GetElement_Get_Double_Element(
        double first,
        Func<double, double> formula,
        int number,
        double expected)
    {
        Assert.That(expected, Is.EqualTo(GetElement(first, formula, number)).Within(DataSourceForTests.Accuracy));
    }
    ...
    public void GenerateSequence_Generate_Integer_Elements_Sequence(
        int first,
        int second,
        Func<int, int, int> formula,
        int count,
        List<int> expected)
    {
        Assert.That(expected, Is.EqualTo(GenerateSequence(first, second, formula, count)));
    }

    [TestCaseSource(typeof(DataSourceForTests), nameof(DataSourceForTests.DataForGenerateSequenceTestForDoubles))]
    [Category(nameof(GenerateSequence))]
    public void GenerateSequence_Generate_Double_Elements_Sequence(
        double first,
        double second,
    ...
    public void CombinePredicates_Combine_String_Predicates(string value, bool status)
    {
        var result = CombinePredicates<string>(
            ...
            x => x.StartsWith("START", StringComparison.InvariantCulture),
            x => x.EndsWith("END", StringComparison.InvariantCulture),
    ...
    {
        Assert.That(expected, Is.EqualTo(FindMax(lhs, rhs, comparer)).Within(DataSourceForTests.Accuracy));
    }
    ...
    public void Calculate_Calculate_For_Integer_Elements(
        int first,
        Func<int, int> formula,
    ...
    [Category(nameof(Calculate))]
    public void Calculate_Calculate_For_Double_Elements(
        double first,
        Func<double, double> formula,
